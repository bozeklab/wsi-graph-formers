...
class DIFFormer_bin(nn.Module):
    '''
    DIFFormer model class (binary head)
    x: input node features [N, D]
    edge_index: 2-dim indices of edges [2, E]
    return y_hat predicted logits [N, 1]
    '''
    def __init__(self, in_channels, hidden_channels, out_channels,  # out_channels kept for signature parity
        ...
        for i in range(num_layers):
            self.convs.append(
                DIFFormerConv(hidden_channels, hidden_channels, num_heads=num_heads, kernel=kernel,
                              use_graph=use_graph, use_weight=use_weight, graph_weight=graph_weight,
