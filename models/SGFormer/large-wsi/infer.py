...
def infer(cfg: DictConfig):
    ...
    infergraph = load_dataset(cfg.data_dir, cfg.infergraph, cfg.sub_dataset)
    ...
    print(f"\ninputgraph {cfg.infergraph} | num nodes {n} | num edge {e} | num node feats {d} | num classes {c}")
    ...
    with torch.no_grad():
        out = model(infergraph.graph['node_feat'].to(device), infergraph.graph['edge_index'].to(device))
        pred = out.argmax(dim=1)

        noext_modelname = os.path.splitext(cfg.model_name)[0] 
        saving_path = cfg.pred_dir + f'predictions_{cfg.infergraph}_{noext_modelname}.pt'
