...
class NodeFormer_bin(nn.Module):
    '''
    NodeFormer model for binary node classification.
    Returns: node logits (one per node), and a list of edge losses at every layer (if enabled).
    '''
    def __init__(self, in_channels, hidden_channels, out_channels,  # out_channels kept for signature parity
        ...
        if use_jk:
            self.fcs.append(nn.Linear(hidden_channels * num_layers + hidden_channels, 1))
        else:
            self.fcs.append(nn.Linear(hidden_channels, 1))  # <- binary head: 1 logit
    ...
    def forward(self, x, edge_index, tau=1.0):
        ...
        x_out = self.fcs[-1](z).squeeze(0)  # [N, 1] logits
